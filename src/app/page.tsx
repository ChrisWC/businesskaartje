...
const Languages = [
  ...
  {
    name: "Japanese, Beginner (A0) via self-study",
    level: "Beginner (A0)",
    source: "self-study",
    icon: {
      attribution: "Various, Public domain, via Wikimedia Commons",
      image: "/japan.svg"
    },
    cert: null
  },
    ...
    <ul className={`${DETAILED_SECTION_LIST_CLASSNAME} pl-${depth > 0? 6: 0} ${depth > 0? "border-l-2":""}`} data-depth={depth}>
      {
        props.list.filter(item => props.filter(item.tags || [])).map(item => (
          <li key={item.name}>
            <div className="flex items-center space-x-4 shadow-md rounded-lg flex-wrap min-w-0">
              <div className="relative w-16 min-h-16 max-h-16">
                <Image
                  aria-hidden
                  src={item.icon!.image}
    ...
    <div className="grid grid-rows-[20px_1fr_20px] items-center justify-items-center min-h-screen p-8 pb-20 gap-16 sm:p-20 font-[family-name:var(--font-geist-sans)] w-full overflow-hidden">
      <main className="flex w-full max-w-full overflow-hidden flex-col gap-8 row-start-2 items-center sm:items-start">
        <div className="flex flex-row gap-8 w-full max-w-full">
          <Image aria-hidden src="/profile_pic.jpeg" alt="handsome man" width="128" height="128" />
          <div>
      ...
      <footer className="flex w-full max-w-full row-start-3 flex gap-6 flex-wrap items-center justify-center">
        <div className="flex w-full max-w-full gap-6 items-center justify-center w-screen">
          <a
